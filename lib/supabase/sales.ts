export type Customer = { id: string; fullName: string; phone?: string; email?: string; documentNumber?: string; notes?: string; createdAt: string };
...
export type SaleProduct = { id: string; name: string; price: number; stock: number; active: boolean; category?: string; connectorType?: string; chipType?: string; voltage?: string; watts?: number; lumens?: number; warrantyDays?: number };
...
export function createCustomer(input: { fullName: string; phone?: string; email?: string; documentNumber?: string; notes?: string }) { return request<Customer>("/api/admin/sales", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "create_customer", ...input }) }); }
...
export type InstallationDraft = { scheduledAt?: string; location?: string; contactPhone?: string; workType?: string; difficulty?: "low" | "medium" | "high" | ""; technician?: string; notes?: string };
export type WarrantyDraft = { productId: string; enabled: boolean; days?: number; startsAt?: string; notes?: string };
export function createSale(input: { customerId: string; customerVehicleId?: string; notes?: string; items: Array<{ productId: string; quantity: number }>; createInstallation?: boolean; installation?: InstallationDraft; warranties?: WarrantyDraft[]; paymentMethod: string; idempotencyKey: string }) { return request<{ saleId: string }>("/api/admin/sales", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "create_sale", ...input }) }); }
