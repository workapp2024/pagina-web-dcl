...
export function ProductClassificationEditor({ product, onSaved, onDraftChange, onPendingChange, onBusyChange }: { product: Product; onSaved: (value: Classification) => void; onDraftChange?: (value: Classification) => void; onPendingChange?: (pending: boolean) => void; onBusyChange?: (busy: boolean) => void }) {
  ...
  return <fieldset disabled={busy} className="rounded-2xl border border-white/10 bg-black/20 p-4">
    ...
    <p className="mb-4 text-xs text-zinc-400">Clasificación comercial; no confirma compatibilidad técnica. {onDraftChange ? "Se guardará al crear el producto." : "Se guarda por separado del resto del producto."}</p>
    <label className="block text-sm">Categoría
      <select value={value.category} onChange={event => change({ category: event.target.value })} className="mt-2 min-h-12 w-full rounded-xl bg-zinc-900 px-3">
    ...
    <div className="mt-4 grid gap-4 sm:grid-cols-2">
      ...
      <fieldset><legend className="text-sm font-bold">Tipo de vehículo{accessory ? " (opcional, selección múltiple)" : " (selección múltiple)"}</legend>
        {accessory && <>
          ...
          <label className="flex min-h-11 items-center gap-3 text-sm"><input type="checkbox" checked={!value.vehicleTypes?.length} onChange={() => change({ vehicleTypes: [] })} />Universal / Todos los vehículos</label>
        </>}
        {productVehicleTypes.map(option => <label key={option.id} className="flex min-h-11 items-center gap-3 text-sm">
        <input type="checkbox" checked={value.vehicleTypes?.includes(option.id) ?? false} onChange={event => change({ vehicleTypes: event.target.checked ? [...(value.vehicleTypes ?? []), option.id] : (value.vehicleTypes ?? []).filter(item => item !== option.id) })}/>{option.label}
      </label>)}</fieldset>
    </div>
    ...
    {!onDraftChange && <button type="button" disabled={busy || !Object.keys(changes).length} onClick={() => void save()} className="mt-4 min-h-11 rounded-full bg-red-600 px-5 text-xs font-bold uppercase disabled:opacity-40">{busy ? "Guardando…" : "Guardar clasificación"}</button>}
