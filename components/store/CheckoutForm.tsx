...
export function CheckoutForm({transfer}:{transfer:Transfer}){
  ...
  const [transferState,setTransferOrder]=useState<{orderId:string;orderNumber:string;amount:number;fingerprint:string}|null>(null);
  ...
  async function createOrder(){
    ...
    return body as {orderId:string;orderNumber:string;total:number;publicKey:string|null};
  }
  async function submit(event:React.FormEvent){
    ...
    try {
      ...
      } else setTransferOrder({orderId:order.orderId,orderNumber:order.orderNumber,amount:order.total,fingerprint});
