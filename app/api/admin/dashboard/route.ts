...
export async function GET() {
  ...
  try {
    ...
    const [sales, orders, products, installations, claims, customers] = await Promise.all([
      db.from("sales").select("id,total,created_at").eq("status", "completed").gte("created_at", today.toISOString()),
      ...
      db.from("customers").select("id", { count: "exact", head: true }).gte("created_at", today.toISOString()),
    ]);
    ...
    return NextResponse.json({ ok: true, data: { todaySales: rows.length, todayRevenue: revenue, estimatedProfit: revenue - costs, pendingOrders: (orders.data || []).length, lowStock: (products.data || []).filter((row: any) => row.stock <= row.stock_min), pendingInstallations: (installations.data || []).length, openClaims: (claims.data || []).length, newCustomers: customers.count || 0, topProducts: [...byProduct.entries()].sort((a, b) => b[1] - a[1]).slice(0, 5), recentSales: rows.slice(0, 6) } });
