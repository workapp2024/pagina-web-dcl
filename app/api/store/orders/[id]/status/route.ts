...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const approved=order.status==="paid"||order.status==="completed"||transaction?.status==="approved";
  ...
  return NextResponse.json({ok:true,result:approved?"approved":rejected?"rejected":"pending",status:order.status,paymentMethod:order.payment_method,total:Number(order.total),currency:order.currency,reference:id.slice(0,8).toUpperCase()},{headers:{"Cache-Control":"no-store"}});
}
